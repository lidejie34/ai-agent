...
/**
 * 标签维度管理端接口（迭代10；自 DimAdminController 拆分）：{@code /api/admin/dim/tags}
 * 派生视图列表 + 改名/删除联动文档。
 *
 * <p><b>常驻装配</b>：与 KbAdminController 同模式——控制器无条件在组件扫描内，
 * {@code app.rag.enabled=false} 时先被 {@code AdminAuthInterceptor} 路径闸门
 * （dim/tags → Gate.RAG）挡为 503 KB_DISABLED；service 缺席仅作防御性 503。
 * 鉴权（X-Admin-Token）全部在拦截器，本类不重复鉴权。
 */
@RestController
@RequestMapping("/api/admin/dim/tags")
public class DimTagAdminController {
    ...
    private final ObjectProvider<DimTagService> serviceProvider;

    public DimTagAdminController(ObjectProvider<DimTagService> serviceProvider) {
    ...
    private DimTagService service() {
        DimTagService service = serviceProvider.getIfAvailable();
        if (service == null) {
