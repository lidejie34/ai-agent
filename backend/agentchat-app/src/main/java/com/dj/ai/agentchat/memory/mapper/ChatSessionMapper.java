...
public interface ChatSessionMapper extends BaseMapper<ChatSessionPO> {
    ...
    /**
     * 首轮标题 best-effort 写入（迭代4 FR-4）：仅当 title 仍为 NULL 时写入，
     * 后续轮次/人工重命名后 title 非 NULL → 匹配 0 行、不覆盖（幂等）。
     */
    @Update("UPDATE chat_session SET title = #{title} "
            + "WHERE session_id = #{sessionId} AND title IS NULL")
    int updateTitleIfAbsent(@Param("sessionId") String sessionId, @Param("title") String title);
}
