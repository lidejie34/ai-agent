...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleSessionNotFound(SessionNotFoundException e) {
        // 会话 REST 接口：UUID 合法但库中无对应行 → 404（前端据 SESSION_NOT_FOUND 移除侧边栏项）
        log.warn("会话不存在: {}", e.getMessage());
        return build(HttpStatus.NOT_FOUND, "SESSION_NOT_FOUND", e.getMessage());
    }
    ...
    public static ApiError toApiError(Throwable e) {
        ...
        if (e instanceof SessionNotFoundException) {
            // 防御性对齐：SSE 聊天路径目前不产生 404（续接未知 UUID 自愈补行），仅会话 REST 接口抛出
            return new ApiError("SESSION_NOT_FOUND", e.getMessage(), now());
        }
