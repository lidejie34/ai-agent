...
迭代7 ✅ 对话内排障三件套（SCRIPT MVP）：skyeye_query_log（uk 简称解析/首查单 uk/expand=1
   ...
   ↓
迭代8 ✅ 工具证据跨轮记忆：工具调用结果随轮落库回放（role='tool_evidence' 零 DDL），
   根治"结合刚才的日志"类追问的上下文丢失与重复工具调用；证据不挤占 20 条对话窗口、
   前端/SSE 零改动、开关默认关闭（735 测试全绿；真实冒烟 7 项全 PASS）
   ↓
迭代9 ✅ 可观测性地基：traceId 全链路（HTTP→模型→工具，复用为工具 requestId 零 DDL 关联审计）
   + Micrometer 内存指标（模型耗时 Timer、token 自采 usage、usage 缺失显式降级不估算）
   + 慢请求环形清单 + actuator 只读端点（零暴露占位 id 起步，按需白名单放开）；
   总开关默认关、关闭态逐字节一致（784 测试全绿；真实冒烟 6 项全 PASS）
   ↓
迭代10 ✅ 知识库标签/项目维度 + 维度维护：文档打标签/项目元数据，检索按维度过滤
   （对话请求 kbProject/kbTags + 常驻 RagAdvisor 过滤 + 管理端列表过滤/编辑标签）；
   追加「维度维护」页：dim_project 受管项目表 CRUD（改名联动文档、引用中禁删 409）、
   标签派生列表改名/删除联动、上传强制受管项目、聊天页维度下拉选择器
   （883 后端 + 228 前端测试全绿；真实冒烟 17 项全 PASS）
```
...
| 迭代 8 | 工具证据跨轮记忆 | 工具调用结果（skyeye 日志/codegraph 代码/QA 库查询）随轮落库回放，根治跨轮上下文丢失（实证：agent_tool_call_log id=114≡107 同入参重复查询）：请求级 `ToolCallBridge` 挂可选 `ToolEvidenceCollector`，证据随 `remember()` 幂等缓存原子记录（Flux 重订阅重试天然不重复）；`CachedOutcome` 扩 6 字段保留单参紧凑构造；新增 `ToolEvidenceMessage` 载体以 `role='tool_evidence'` 插在 user 与 assistant 之间落库（**零 DDL**，role VARCHAR(16) 无 CHECK），回放为带「【此前工具查证记录】」前缀的 AssistantMessage（只门控写侧，存量证据行关开关仍回放）；渲染每轮一条消息每次调用一行（单条 800/整轮 4000 截断+省略行，失败/TIMEOUT 照记）；`selectRecent`/`selectLatestPerSession` 改白名单窗口 SQL（内层 user/assistant/system 倒序 LIMIT 定 min_id、外层取全角色——证据不挤占 20 条窗口，无证据会话逐行回归一致）；`SessionService.listMessages` 白名单过滤（前端/SSE 零改动、无新事件类型）；`app.chat.evidence.{enabled:false,max-chars-per-call:800,max-chars-per-turn:4000}`；ChatService 13 参主构造 + 保留 12 参便捷构造传默认关实例（既有测试零改动） | ✅ 已交付（后端 **735** 测试全绿：704 既有零回归 + 31 新增——Collector 9/Bridge 6/Memory 真实库 4/SessionService 真实库 3/ChatService 9，既有 Db/McpToolCallbackTest 各 +3；真实库窗口语义回归 + 两条新 SQL EXPLAIN 走 idx_session；分支 feat/tool-evidence-memory，commit 3aa1477 已合入 master；**真实冒烟 7 项全 PASS**：轮 1 落 user/tool_evidence/assistant 三行、轮 2 追问零重复工具调用（对照 id=114≡107 实证）、出站消息数=4 证实证据回放、无工具轮不落证据、历史接口无 tool_evidence、enabled=false 回滚一致、连续 assistant 回放未 400） | `20260914-ai-project-tool-evidence-memory` |
| 迭代 9 | 可观测性地基（③） | **traceId 全链路 + Micrometer 内存指标 + 慢请求清单**：`TraceIdFilter`（OncePerRequestFilter 含 /api/admin）统一生成 16-hex traceId，`X-Trace-Id` 响应头回传、MDC 注入日志（`logging.pattern.level` 可加 `%X{traceId}`）、经 context-propagation + 三 daemon 池条件包裹跨 Reactor/线程池传递；工具层 `requestId` 复用 traceId（`agent_tool_call_log.call_id` 前缀直接关联，**零 DDL**）；SSE 既有 session 帧附带 traceId 字段（不新增帧类型）；`ObservabilityMetricsAdvisor`（ORDER=10，Call+Stream 双路径）录 `dj.chat.model.call.duration{model,caller,path,outcome}` Timer + `dj.chat.model.tokens.prompt/completion/total` 自采 usage，流式拿不到记 `dj.chat.model.usage.missing` 显式降级**不估算**（streamUsage 子开关默认 false，Ark 容忍度待联调）；`SlowRequestTracker` 环形 100 条（阈值默认 30s 外置）+ `@Endpoint(slowrequests)` 只读端点；引入 spring-boot-starter-actuator 3.4.0（已回填 tc_resp 离线可编译）+ context-propagation 1.1.2，**不引 Prometheus**；R-2 暴露面控制：`exposure.include` 占位 id `observability-none`（空串会回落 Boot 默认暴露 health）+ `discovery.enabled=false`，关闭态 `/actuator/**` 与任意未知路径同构 500（GlobalExceptionHandler 归一）；总开关 `app.observability.enabled` 默认 false 零装配，既有类全走新构造+旧构造委托默认关（既有测试零改动）；M7 `spec.options()` 整体替换实证 → streamUsage 挂载点在 Advisor 内 `AdvisedRequest.from().chatOptions(copy)` 改写 | ✅ 已交付（后端 **784** 测试全绿：735 既有零回归 + 48 新增——TraceIds 5/Filter 4/Tracker 7 含 8×100 并发/MetricsAdvisor 12/条件装配 5/SessionEvent 逐字节 3/MDC 传播 3/R-2 专项 2 等；分支 feat/observability-foundation，commit f2fb674 已合入 master；**真实冒烟 6 项全 PASS**：X-Trace-Id 16-hex 响应头、34 行日志同一 traceId 全链串联且审计 call_id 前缀=traceId、token meter 17663+80=17743 自洽、慢请求端点含完整条目、SSE session 帧带 traceId + 流式 missing 计数、关闭态 R-2 同构 500 且响应头无 traceId） | `20260914-ai-project-observability-foundation` |
| 迭代 10 | 知识库标签/项目维度 + 维度维护 | **主范围（429f131）**：`rag_document` 增 `project VARCHAR(64)`/`tags JSONB` 列 + project 普通索引/tags GIN 索引（rag-schema.sql 幂等追加）；上传/编辑元数据打标（KbMetaValidator：项目/标签仅中文/字母/数字/中划线/下划线、标签单 ≤32 最多 8 个、KB_INVALID_PROJECT/KB_INVALID_TAGS 400 透传）；检索过滤三入口——对话请求体 `kbProject`/`kbTags`（三态：null 不过滤/空串或空数组不过滤/有值过滤）、常驻 RagAdvisor 在 pgvector 余弦检索 SQL 上叠加 `project = ? AND tags ??| array[...]` 维度条件（同步/SSE/SDD Executor 三处继承零改动）、管理端文档列表 project/tag 过滤；前端知识库页过滤区 + 上传/编辑弹窗打标字段。**追加范围（21e238e + MySQL 迁移，用户要求项目受管为后续工具调度复用）**：`dim_project` 受管项目表（id/name UNIQUE/remark/双时间戳，**MySQL 主库** db/dim-schema.sql——初版曾建 PG，为「RAG 关闭也能维护项目/供 MySQL 侧工具调度复用」迁至主库，无条件装配 DimConfig + 懒建表复刻 ChatMemorySchemaInitializer；rag_document.project 保持 name 字符串松耦合不建跨库外键）；`/api/admin/dim` 管理端点——项目 CRUD（重名 409 KB_PROJECT_EXISTS 预检+唯一键双保险、改名先 PG 文档联动后 MySQL 行+失败补偿回改、引用中删除 409 KB_PROJECT_IN_USE 含引用数、不存在 404）、标签派生列表（jsonb_array_elements_text GROUP BY）改名/删除（jsonb_agg DISTINCT 去重写回/tags - 'x' 删除，返回 affectedDocs）；docCount 跨库应用层合并（MySQL 项目 + PG GROUP BY 计数，RAG 关闭降级 0）；AdminGateResolver dim 二级分派（tags→Gate.RAG、projects→TOKEN_ONLY，RAG 关闭也能维护项目）；上传/updateMeta 强制 `requireManagedProject`（非 null 项目必须已在 dim_project 创建，否则 400「项目不存在，请先在『维度维护』页创建」，null 跳过校验）；公开 `GET /api/kb/dimensions`（无 admin token，RAG 关闭返回空列表 200 前端静默隐藏）；管理端新增「维度维护」页（项目表格 CRUD + 标签表格改名/删除，409 文案透传），知识库页过滤区/上传/编辑弹窗项目字段 Input→Select 下拉；聊天页 `KbFilterBar`（useKbDimensions 挂载拉取，available 才渲染、流式中禁用），`useChatStream.send(text, kbFilter?)` 仅在有值时附 kbProject/kbTags 键（空过滤与未打标请求体逐字节一致） | ✅ 已交付（后端 **900** 测试全绿：784 既有 + 主范围 52 + 追加/迁移 Dim* 51 + 适配 13；前端 **228** 测试全绿，tsc/build 零错误；**真实冒烟三轮全 PASS**：主范围 7 项发现 pgjdbc 三处问题（JSONB `?` 双写 `??`/null 参数 `?::text` cast/advisor param null 防御）修复于 a7e300e；追加范围 17 项（21e238e）；MySQL 迁移后 13 项——期间实证修复 2 处：**Boot 自动配置 JdbcTemplate 与 ragJdbcTemplate 并存时按类型注入误中 PG**（DimConfig 显式 dimJdbcTemplate bean 绑 @Primary 主库）、**KeyHolder 不指定返回列时驱动返回整行致 getKey() 多列报错**（prepareStatement 显式 `{"id"}` 列名）；RAG 关闭下项目 CRUD 可用/标签 503/dimensions 空，冒烟数据已清理；分支 feat/kb-tag-project-filter 已推送） | `20260915-ai-project-kb-tag-project-filter` |
| 迭代 11 | 聊天页知识库项目多选 | 对话请求体 `kbProject`（单值）升级为 `kbProjects`（数组）：`KbMetaValidator.normalizeProjects`（trim/去重/逐个白名单校验，非法项目 400 KB_INVALID_FILTER 同口径）；`KbFilter`/`RagAdvisor.PARAM_KB_PROJECTS`/pgvector 检索 SQL 项目条件改 `project = ANY(string_to_array(?::text, ','))`（projectsJoined 逗号串 ×2 参数，pgjdbc null cast 纪律沿用）；ChatService/ExecutorClient 三处注入空集合缺省（请求形态最小差异）；前端 `KbFilterBar` 项目 Select `mode="multiple"`（`KbFilterValue{projects,tags}`），`useChatStream`/`sse` 请求体 `kbProjects?: string[]`；6 参 ChatRequest 兼容构造保留（既有调用点零改动） | ✅ 已交付（后端 **906** 测试全绿；前端 **228** 测试全绿，tsc 零错误；真实冒烟 4 组全 PASS：多项目过滤命中注入 4 片段/单项目+标签/列表内含逗号项目 400/无过滤回归；commit 51767a9 已合入 master） | `20260916-ai-project-kb-project-multiselect` |
| 迭代 12 | 会话级工具/MCP 选择 + 范围持久化 | **对话级工具范围**：请求体 `toolNames`/`mcpServers` 三态（null=默认全部/[]=显式全不挂/非空=子集），`ToolSelection` record + `DefaultToolSupport.mountTools(sessionId, selection)` 单挂载点过滤——DB 按工具名、MCP 经 `McpServerConnectionManager.toolCallbacks(Set)` 按 **server key 源头过滤**（MCP server 即组，无新组实体；暴露名 `<server>_<tool>` 归一截断故绝不做前缀匹配）；两侧皆空 → null 挂载不触发 .tools()。**会话级范围持久化**：`chat_session_scope` 表（session_id PK + kb_projects/kb_tags/tool_names/mcp_servers 四列 JSON 串，**NULL vs '[]' 三态逐字节保持**）+ `GET/PUT /api/sessions/{sid}/scope`（kb 两维 KbMetaValidator 白名单 400、toolNames/mcpServers 宽松规整、全量覆盖语义、级联删除随行）；写路径双通道——选择器变更防抖 PUT + 有状态轮请求体 best-effort upsert（首轮绑定兜底、PUT 失败漂移自愈），无状态轮零交互；**模型链纯请求驱动**（NFR-1：持久化值只服务 UI 回填，服务端绝不回灌请求）；公开 `GET /api/tools/available`（常驻装配，任一侧缺席/异常降级空列表 200）；前端 `ToolScopeBar`（「启用工具」开关 + 内置工具/MCP server 多选，仅 READY server 可选）、会话切换 `getSessionScope` 回填（`enabled = !(toolNames==[] && mcpServers==[])`）、用户 onChange 防抖 PUT（恢复绝不回写防回环）、工具不可用时选择器隐藏且请求体不带工具键（与迭代 11 逐字节一致） | ✅ 已交付（后端 **947** 测试全绿：906 既有 + 41 新增/适配——选择矩阵 8/MCP Set 过滤 2/真实库 scope 4/mock 委派 4/SessionService 7/Controller 切片 5+4/ChatService 6/JSON 2/schema 计数适配等；前端 **237** 测试全绿：228 既有 + 9 新增，tsc 零错误；**真实冒烟 6 组全 PASS、零冒烟修复**：available 端点 1 DB+6 READY server、子集挂载 15=1+14、首轮绑定 upsert、GET/PUT/400 协议、无状态轮零交互+第二轮不覆写、无 scope 回归全量 56+级联删除；期间修复 jar 未重打包冒烟假象 1 处） | `20260916-ai-project-session-scope-config` |
| 热修复 | SSE 流式超时三缺陷（D1/D2/D3） | 用户长生成（471 片段）120s 被截断且日志 ERROR「未处理异常」实证三缺陷链：**D1** `app.chat.sse-timeout-ms` 默认 120000→`${APP_CHAT_SSE_TIMEOUT_MS:600000}`（@Value 兜底同步 600000）；**D2** 超时终态帧——实施中字节码实证 **Spring 6.2 容器超时回调入口 `ResponseBodyEmitter$DefaultCallback.run()` 先 CAS 置 complete 再执行 onTimeout 委托，回调内 send 必抛 IllegalStateException**，改 Reactor 软截止：`ChatController.withSoftDeadline` 包 `take(max(超时-5s, 超时/2))` + doOnComplete 置位 + concat 空/错误流——截止走既有 flux 错误消费器发 `event:error`（新错误码 `SSE_TIMEOUT`「模型响应超时，已生成内容可能不完整」）再 complete，自然完成不误伤；`SseStreamTimeoutException` 经 `GlobalExceptionHandler.toApiError` 映射；错误消费器对该异常慢请求终态记 `timeout`（口径不变）；容器 onTimeout 退化为纯兜底清理不发帧；**D3** `@ExceptionHandler(AsyncRequestTimeoutException.class)` 返回 null ResponseEntity（Spring 视为已处理零字节写入，debug 级日志）——不再 ERROR、不再把 INTERNAL_ERROR JSON 追加进 SSE 字节流（软截止正常路径本不触发，双保险）；前端零改动（既有 error 帧路径标错误态、正文保留） | ✅ 已交付（后端 **953** 测试全绿：947 既有 + 6 新增——ChatControllerTimeoutTest 3（软截止发帧+片段保留+timeout 记录/自然完成回归/容器兜底清理）、GlobalExceptionHandlerAsyncTimeoutTest 3；前端 **237** 全绿零改动，tsc 零错误；**真实冒烟两轮 + 默认值验证全 PASS**：8s 硬超时→4s 软截止客户端收到 SSE_TIMEOUT 终态帧（71 片段保留）、流尾无 INTERNAL_ERROR JSON、后端无 ERROR/无 AsyncRequestTimeoutException、默认 600s 生效且短轮 done 帧正常；分支 fix/sse-stream-timeout） | `20260916-ai-project-sse-timeout-fix` |
| 热修复 | 对话页三下拉「都不加载」互斥选项 | KB 项目/内置工具/MCP 服务三多选各加首项「都不加载」（用户原话「只是都不加载」弹窗澄清扩展到三下拉）：**KB 后端新三态**——显式 `kbProjects:[]` = 本轮完全不加载知识库（缺键=默认全部/非空=子集不变），`KbFilter` 加 `disabled` 组件 + `none()` 工厂（两参兼容构造既有调用点零改动），ChatService validate/resolve 区分 null vs []（[] 时 kbTags 宽松忽略不 400，NFR-3），`RagAdvisor.PARAM_KB_DISABLED` 挂载后 `augment()` 开头直接放行（不 embedding/不 search/不注入，类型异常防御为未禁用），同步/流式/SDD Executor 三处挂载点一致；**工具链后端零改动**（迭代12 []=该侧全不挂语义天然覆盖）；**前端三哨兵** `scopeNone.ts`（KB_NONE/DB_TOOLS_NONE/MCP_NONE 单一定义）+ 三处翻译层——发送（哨兵→显式 []、KB 哨兵时 tags 省略）、scope 保存（哨兵→[] 不归一为 null）、scope 恢复（kbProjects:[]→显哨兵+标签禁用、单侧 []→该侧显哨兵、两侧皆 []→仍推导 enabled=false 迭代12 兼容）；选择器内互斥（选哨兵清空同侧其他项+KB 侧禁用标签下拉，再选具体项自动摘除哨兵）；`chat_session_scope` 的 '[]' 存取迭代12 已支持，持久化零改动 | ✅ 已交付（后端 **965** 测试全绿：953 既有 + 12 新增——RagAdvisorDisabledTest 4/ChatServiceKbDisabledTest 5/ExecutorClientKbFilterTest +3；前端 **244** 测试全绿：237 既有 + 7 新增，tsc 零错误；**8091 真实冒烟 5 轮全 PASS、零冒烟修复**：kbProjects:[] 跳过检索（debug 日志实证+回答无参考资料）vs 缺键对照检索注入 4 片段、toolNames:[] 工具数=55（MCP only）、mcpServers:[] 工具数=1（DB only）、scope 三态持久化往返 [] 不归一为 null；期间修复 jsdom rc-motion 离场动画 pointer-events 测试竞态 1 处；分支 feat/selector-none-option） | `20260916-ai-project-selector-none-option` |
| 热修复 | 对话界面美化（现代简约浅色） | 用户反馈「对话界面有点丑」，弹窗确认四大区域全美化 + 现代简约浅色风格；**零新依赖、纯 CSS 为主**（index.css 重构 + App.tsx 一处包裹 div + MessageBubble 三点波浪 + 文案 2 处）：气泡改不对称尾角圆角 14px + 柔和投影替代硬边框（用户蓝 `0 2px 8px rgba(22,119,255,.25)` / 助手白 `0 1px 6px`）、KB/工具筛选条收纳为白底圆角 12px 轻阴影「范围工具条」`.chat-scope-bar`（KB 或工具任一可用即渲染、均不可用缺席）、输入区去硬边框改柔和分隔阴影 + 控件圆角 8px、侧边栏 hover 底色 + 选中项左侧 3px 蓝色指示条 + 操作按钮 hover/focus-within 显现（键盘可达）、空态标题蓝紫渐变文字 + 示例卡 hover 上浮、代码块细边框圆角、全局 6px 细滚动条；全部 data-testid 与交互行为零改动，管理控制台样式零波及 | ✅ 已交付（前端 **245** 测试全绿：244 既有 + 1 新增（scope-bar 渲染条件两阶段断言）；tsc 零错误、vite build 通过；5174 dev 冒烟 PASS：页面 200 / 新 CSS 类正常服务 / /api 代理至 8080 正常；分支 feat/chat-ui-beautify） | `20260916-ai-project-chat-ui-beautify` |
| 迭代 13 | 对话删除 + 上下文删除 | **消息级删除**：气泡 hover/focus 浮现「删除本轮」（user 锚点成对删用户消息+AI 回复，区间 `[userId, nextUserId)` 一跳 SQL 覆盖区间内 tool_evidence）与「截断重问」（删该条及之后全部、内容回填输入框），物理删除 + Popconfirm 防误触；**清空上下文但保留记录**（FR-5）：零 DDL 复用迭代8 先例插 `role='context_reset'` 标记行，`selectRecent` 内层窗口与外层取回统一 `id > COALESCE(MAX(标记id),0)`——无标记会话逐字节回归、标记行天然不回放，MybatisChatMemory.get 单点覆盖同步/流式/SDD 三链路；**删除/截断跳过标记行**（记忆边界不随时光倒流复活）；**批量删会话**：`POST /api/sessions/batch-delete` 逐 id 汇报 deleted/notFound 不打断整批，侧边栏批量管理模式（勾选+操作条）；**主区入口**：顶栏「⋯」Dropdown 收纳清空上下文/删除当前会话（Modal 确认）；**消息身份**：`SessionMessageView` 带库 `id` + `context_reset` 放行（排除式过滤，证据行仍拦截），前端直播轮次结束后从尾向前按 role+content 双等静默对齐回填 backendId（防错误轮次错位，对齐失败降级不显示操作）；分隔线「上下文已清空 · 以上历史不再携带」渲染记忆边界；SSE 零新帧、流式中全部操作禁用 | ✅ 已交付（后端 **992** 测试全绿：965 既有零回归 + 27 新增——SessionServiceMessageOpsTest 13/MybatisChatMemoryContextResetTest 7 真实库/SessionControllerTest +7；前端 **258** 测试全绿：245 既有 + 13 新增，tsc 零错误、vite build 通过；**8091+5174 真实冒烟全 PASS 零修复**：删第二轮模型不再记得 222 仍记得 111/333、截断后仅余首轮、清空后模型自述「这是你的第一条消息」+ GET 含 context_reset 分隔、batch-delete 混合 deleted/notFound/级联零残留、非 user 锚点 400/跨会话 404、vite 代理新端点端到端通；分支 feat/conversation-context-delete） | `20260917-ai-project-conversation-context-delete` |
...
| 可观测性（③） | Spring AI Observation（token 用量/模型耗时）、traceId、慢请求统计；Micrometer | ✅ 迭代 9 已落地（内存指标+actuator 只读端点，不引 Prometheus；流式 usage 缺失显式降级） |
...
- **【迭代7.2 实证】方舟入口隔离**：Coding Plan 端点 `/api/coding/v3`（月度配额，耗尽后大请求
  429 AccountQuotaExceeded、小请求仍能挤过，重置周期见报错）；AgentPlan 端点 `/api/plan/v3`
  （key 按入口隔离——AgentPlan key 打 /api/coding/v3 或 /api/v3 均 401，打 /api/plan/v3 大小请求
  均 200）；`/api/v3` 按量计费未激活模型报 404 ModelNotOpen。当前本地配置：AgentPlan key +
  base-url `/api/plan/v3` + model `ark-code-latest`（application-local.yml，2026-09-14）
- **【迭代8 实证】跨轮上下文丢失根因**：ChatService 只落 UserMessage+最终 AssistantMessage，
  Spring AI M7 内部 tool-calling 循环的工具结果仅存内存；下一轮 store.get(sid,20) 回放无工具
  证据 → 模型把相同查询原样重跑（实证 agent_tool_call_log id=114 与 107 入参完全一致、结果
  4392 字符分毫不差）。迭代8 以 role='tool_evidence' 随轮落库修复；存量证据行如需抹除：
  `DELETE FROM chat_message WHERE role='tool_evidence'`
- **【迭代9 实证】actuator 暴露面控制**：`management.endpoints.web.exposure.include` 置**空串**
  会被绑定为"未设置"回落 Boot 默认（health 实暴露）；零暴露须用不存在的占位 id
  （如 `observability-none`）+ `management.endpoints.web.discovery.enabled=false`；
  本应用 GlobalExceptionHandler 把任意未知路径归一为 500 INTERNAL_ERROR，未暴露的
  /actuator/** 与之字节同构（无 404 语义）；冒烟期放开用环境变量
  `APP_OBSERVABILITY_ACTUATOR_INCLUDE=health,metrics,slowrequests`（2026-09-15）
- **【迭代9 实证】M7 `DefaultChatClientRequestSpec.options(T)` 是整体替换非合并**（javap）：
  在 spec 上直接挂自定义 options 会丢 model/temperature/reasoning-effort；定制
  `OpenAiChatOptions.streamUsage` 须在 Advisor 内 `AdvisedRequest.from(req).chatOptions(副本).build()` 改写
- **【迭代9 实证】方舟 ark-code-latest 用量回传**：同步 call 回传完整 usage
  （实测 prompt=17663 + completion=80 = total=17743 自洽）；流式默认无 usage
  （记 `dj.chat.model.usage.missing` 降级，不估算）；`stream_options.include_usage` 对方舟的
  容忍度未联调——`app.observability.stream-usage-enabled` 子开关默认 false，验证前勿开；
  actuator 3.4.0 三 jar 已回填 ~/tc/tc_resp（mvn -o 离线编译通过）
- **【迭代10 实证】pgjdbc 三大坑**（冒烟实测，写 PG SQL 必遵）：① JSONB 键存在运算符 `?`
  与 JDBC 占位符冲突，SQL 文本中必须双写 `??`（如 `tags ?? ?::text`）；② 可能为 null 的参数
  或处在运算符上下文（如 `??`、`??|`、`->>` 右侧）的参数必须显式 cast（`?::text`/`?::varchar`），
  否则 pgjdbc 推不出类型报 "could not determine data type"；③ Spring AI M7 Advisor 的
  `param(...)` 不允许 null 值，过滤参数为空时须不挂该 param 而非传 null
- **【迭代10 实证】多数据源下 JdbcTemplate 注入陷阱**（冒烟实测事故）：RAG 开启时容器同时有
  Boot 自动配置 JdbcTemplate（@Primary MySQL）与 ragJdbcTemplate（PG），按类型注入
  有歧义风险（实测误中 PG——pgjdbc getGeneratedKeys 返回整行暴露）。教训：多数据源工程里
  每个存储侧都显式声明专属 JdbcTemplate bean + @Qualifier 注入，不依赖 Boot 自动配置；
  KeyHolder 取自增 id 必须 `prepareStatement(sql, new String[]{"id"})` 显式指定返回列，
  否则驱动返回整行时 getKey() 报 "multiple keys"
- **【迭代10 实证】JSONB 标签数组改名/删除 SQL**：改名用
  `jsonb_agg(DISTINCT CASE WHEN v #>> '{}' = from THEN to_jsonb(to::text) ELSE v END)`
  子查询整体重写 tags 列（DISTINCT 防止目标标签已在数组中产生重复），删除用 `tags - 'x'::text`；
  派生标签列表用 `jsonb_array_elements_text(d.tags) t(name) GROUP BY name`
