...
迭代8 ✅ 工具证据跨轮记忆：工具调用结果随轮落库回放（role='tool_evidence' 零 DDL），
   ...
   前端/SSE 零改动、开关默认关闭（735 测试全绿；冒烟待执行）
```
